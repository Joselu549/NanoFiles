...
public class NFShell {
  ...
  public String chooseDirectory(String defaultDirectory) {
    ...
    do {
      System.out.print("Do you want to use '" + defaultDirectory
          + "' as location of the directory server? (y/n): ");
      ...
      case NFCommands.COM_DOWNLOAD:
        if (args.length != 2) {
          System.out.println("Correct use:" + NFCommands.commandToString(command)
              + " <filename_substring> <local_filename>");
          return false;
        }
        break;
      case NFCommands.COM_UPLOAD:
        if (args.length != 2) {
          System.out.println("Correct use:" + NFCommands.commandToString(command)
              + " <filename_substring> <remote_server>");
          return false;
        }
        break;
      default:
